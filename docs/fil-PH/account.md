---
title: I-manage ang Subscription
layout: page
---
...
<h1>I-manage ang iyong subscription</h1>
<p class="subtitle">Ilagay ang iyong email para makatanggap ng sign-in link.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">Ipadala ang link</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Tingnan ang iyong email para sa sign-in link. Dadalhin ka nito sa billing portal kung saan maaari mong i-update ang iyong plan, payment method, o mag-cancel.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'May nangyaring mali. Subukan ulit.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Hindi maabot ang server. Subukan ulit.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Ipadala ang link'
  })
