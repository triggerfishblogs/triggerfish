...
<h1>إدارة اشتراكك</h1>
<p class="subtitle">أدخل بريدك الإلكتروني لتلقي رابط تسجيل الدخول.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">إرسال الرابط</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'تحقق من بريدك الإلكتروني للحصول على رابط تسجيل الدخول. سينقلك إلى بوابة الفوترة حيث يمكنك تحديث خطتك أو طريقة الدفع أو الإلغاء.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'حدث خطأ ما. يرجى المحاولة مرة أخرى.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'تعذر الوصول إلى الخادم. يرجى المحاولة مرة أخرى.'
    }
