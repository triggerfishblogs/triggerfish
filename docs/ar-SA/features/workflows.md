...
يمكن لسير العمل أن تتضمن اختيارياً وكيل إصلاح ذاتي مستقل يراقب التنفيذ في الوقت الفعلي، ويشخّص حالات الفشل، ويقترح إصلاحات. عند تمكين الإصلاح الذاتي، يتم إنشاء وكيل رئيسي إلى جانب تشغيل سير العمل. يراقب كل حدث خطوة، ويصنّف حالات الفشل، وينسّق فرق متخصصين لحل المشكلات.

### تمكين الإصلاح الذاتي

أضف كتلة `self_healing` إلى قسم `metadata.triggerfish` الخاص بسير العمل:
...
عند تعيين `enabled: true`، يجب أن تتضمن كل خطوة ثلاثة حقول metadata:

| Field         | الوصف                                          |
| ------------- | ---------------------------------------------- |
| `description` | ما تفعله الخطوة ولماذا توجد                   |
| `expects`     | شكل الإدخال أو الشروط المسبقة التي تحتاجها الخطوة |
| `produces`    | شكل الإخراج الذي تولده الخطوة                  |

يرفض المحلل سير العمل التي تفتقد أي خطوة لهذه الحقول.

### خيارات التكوين

| Option                    | Type    | Default              | الوصف |
| ------------------------- | ------- | -------------------- | ----- |
| `enabled`                 | boolean | —                    | مطلوب. يمكّن وكيل الإصلاح. |
| `retry_budget`            | number  | `3`                  | الحد الأقصى لمحاولات التدخل قبل التصعيد كغير قابل للحل. |
| `approval_required`       | boolean | `true`               | ما إذا كانت إصلاحات سير العمل المقترحة تتطلب موافقة بشرية. |
| `pause_on_intervention`   | string  | `"blocking_only"`    | متى يتم إيقاف المهام التالية مؤقتاً: `always` أو `never` أو `blocking_only`. |
| `pause_timeout_seconds`   | number  | `300`                | الثواني المنتظرة أثناء الإيقاف المؤقت قبل تفعيل سياسة المهلة. |
| `pause_timeout_policy`    | string  | `"escalate_and_halt"`| ما يحدث عند انتهاء المهلة: `escalate_and_halt` أو `escalate_and_skip` أو `escalate_and_fail`. |
| `notify_on`               | array   | `[]`                 | الأحداث التي تُفعّل الإشعارات: `intervention` أو `escalation` أو `approval_required`. |

### كيف يعمل

1. **المراقبة.** يتلقى وكيل الإصلاح الرئيسي تدفقاً في الوقت الفعلي لأحداث الخطوات (بدأت، اكتملت، فشلت، تم تخطيها) أثناء تنفيذ سير العمل.

2. **التصنيف.** عندما تفشل خطوة، يصنّف الوكيل الرئيسي الفشل في واحدة من خمس فئات:

   | الفئة                 | المعنى                                           |
   | --------------------- | ------------------------------------------------ |
   | `transient_retry`     | مشكلة مؤقتة (خطأ شبكة، حد معدل، 503)            |
   | `runtime_workaround`  | خطأ غير معروف لأول مرة، قد يمكن التغلب عليه      |
   | `structural_fix`      | فشل متكرر يحتاج تغيير في تعريف سير العمل         |
   | `plugin_gap`          | مشكلة مصادقة/بيانات اعتماد تتطلب تكاملاً جديداً  |
   | `unresolvable`        | استنفاد ميزانية إعادة المحاولة أو عطل جوهري       |

3. **فرق المتخصصين.** بناءً على فئة التصنيف، ينشئ الوكيل الرئيسي فريقاً من الوكلاء المتخصصين (مشخّص، منسق إعادة محاولة، مصلح تعريف، مؤلف plugin، إلخ.) للتحقيق في المشكلة وحلها.

4. **مقترحات الإصدار.** عند الحاجة إلى إصلاح هيكلي، يقترح الفريق إصداراً جديداً من سير العمل. إذا كان `approval_required` مضبوطاً على true، ينتظر المقترح المراجعة البشرية عبر `workflow_version_approve` أو `workflow_version_reject`.

5. **إيقاف مؤقت محدد النطاق.** عند تمكين `pause_on_intervention`، يتم إيقاف المهام التالية فقط مؤقتاً — تستمر الفروع المستقلة في التنفيذ.

### أدوات الإصلاح

تتوفر أربع أدوات إضافية لإدارة حالة الإصلاح:

| الأداة                     | الوصف                                          |
| -------------------------- | ---------------------------------------------- |
| `workflow_version_list`    | عرض الإصدارات المقترحة/الموافق عليها/المرفوضة |
| `workflow_version_approve` | الموافقة على إصدار مقترح                       |
| `workflow_version_reject`  | رفض إصدار مقترح مع ذكر السبب                  |
| `workflow_healing_status`  | حالة الإصلاح الحالية لتشغيل سير عمل           |

### الأمان

- **لا يمكن لوكيل الإصلاح تعديل تكوين `self_healing` الخاص به**. يتم رفض الإصدارات المقترحة التي تغيّر كتلة التكوين.
- يرث الوكيل الرئيسي وجميع أعضاء الفريق مستوى taint سير العمل ويتصاعدون بالتوازي.
- تمر جميع إجراءات الوكلاء عبر سلسلة hooks السياسة القياسية — لا تجاوزات.
- تُخزن الإصدارات المقترحة عند مستوى تصنيف سير العمل.
