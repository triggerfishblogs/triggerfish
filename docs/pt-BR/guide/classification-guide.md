...
[regla de no write-down](/pt-BR/security/no-write-down) y no se puede anular.
...
- [Regla de no write-down](/pt-BR/security/no-write-down) — la regla fija de flujo de datos
- [Configuración](/pt-BR/guide/configuration) — referencia completa de YAML
- [MCP Gateway](/pt-BR/integrations/mcp-gateway) — modelo de seguridad del servidor MCP
