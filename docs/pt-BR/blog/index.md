...
<div class="blog-index">
  ...
  <article v-for="post of posts" :key="post.url" class="post-card">
    ...
    <div class="post-meta">
      <time :datetime="post.date">{{ (() => { const [y,m,d] = String(post.date).split('T')[0].split('-').map(Number); return new Date(y,m-1,d).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' }); })() }}</time>
