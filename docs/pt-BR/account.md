...
<p class="subtitle">Digite seu e-mail para receber um link de login.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="voce@exemplo.com" />
<button id="account-btn">Enviar link</button>
</div>
...
onMounted(() => {
  // Deriva o gateway a partir do hostname atual (sem URLs hardcoded)
  ...
  // Verifica o parâmetro token — se presente, mostra o painel de uso em vez do formulário de e-mail
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Verifique seu e-mail para um link de login. Ele levará você ao portal de faturamento onde você pode atualizar seu plano, método de pagamento ou cancelar.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Algo deu errado. Por favor, tente novamente.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Não foi possível conectar ao servidor. Por favor, tente novamente.'
    }
