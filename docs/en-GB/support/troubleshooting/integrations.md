...
- Honours the `Retry-After` header from Notion's response
...
The vault or specific folder has a classification level that conflicts with the session taint. See [Security Troubleshooting](/en-GB/support/troubleshooting/security) for details on write-down rules.
