...
Platform-specific behaviour, requirements, and quirks.
...
On servers without a desktop environment, the GNOME Keyring / Secret Service daemon may not be running. See [Secrets Troubleshooting](/en-GB/support/troubleshooting/secrets) for setup instructions.
...
Triggerfish enables Virtual Terminal Processing for coloured console output. This works in modern PowerShell and Windows Terminal. Older `cmd.exe` windows may not render colours correctly.
...
| `TRIGGERFISH_DOCKER` | `true` | Enables Docker-specific behaviour |
...
- **Temp directory not writable.** The wrapper script needs to be written to disc before execution.
