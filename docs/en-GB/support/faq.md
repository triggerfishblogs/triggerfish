...
~/.triggerfish/
  ...
  TRIGGER.md                # Proactive behaviour definition
...
Yes. Set the `TRIGGERFISH_DATA_DIR` environment variable to your desired path before starting the daemon. If you are using systemd or launchd, you will need to update the service definition (see [Platform Notes](/en-GB/support/guides/platform-notes)).
...
Values prefixed with `secret:` are references to your OS keychain. At startup, Triggerfish resolves each reference and replaces it with the actual secret value in memory. The raw secret never appears in `triggerfish.yaml` on disk. See [Secrets & Credentials](/en-GB/support/troubleshooting/secrets) for backend details by platform.

### What is SPINE.md?

`SPINE.md` is your agent's identity file. It defines the agent's name, mission, personality, and behavioural guidelines. Think of it as the system prompt foundation. The setup wizard (`triggerfish dive`) generates one for you, but you can edit it freely.

### What is TRIGGER.md?

`TRIGGER.md` defines your agent's proactive behaviour: what it should check, monitor, and act on during scheduled trigger wakeups. Without a `TRIGGER.md`, triggers will still fire but the agent will have no instructions for what to do.
...
4. **Is the owner ID correct?** If you are not recognised as the owner, the bot may restrict responses

See the [Channels Troubleshooting](/en-GB/support/troubleshooting/channels) guide for channel-specific checklists.

### What is the owner ID and why does it matter?

The owner ID tells Triggerfish which user on a given channel is you (the operator). Non-owner users get restricted tool access and may be subject to classification limits. If you leave the owner ID blank, behaviour varies by channel. Some channels (like WhatsApp) will treat everyone as the owner, which is a security risk.
...
Then restart the daemon. See [LLM Provider Troubleshooting](/en-GB/support/troubleshooting/providers) for provider-specific guidance.
...
A skill is a folder containing a `SKILL.md` file that gives the agent new capabilities, context, or behavioural guidelines. Skills can include tool definitions, code, templates, and instructions.
...
Triggers are periodic agent wakeups for proactive behaviour. You define what the agent should check in `TRIGGER.md`, and Triggerfish wakes it on a schedule. The agent reviews its instructions, takes action (check a calendar, monitor a service, send a reminder), and goes back to sleep.
...
Agent teams are persistent groups of collaborating agents that work together on complex tasks. Each team member is a separate agent session with its own role, conversation context, and tools. One member is designated the lead and coordinates the work. See [Agent Teams](/en-GB/features/agent-teams) for full documentation.
...
The most common cause is a misconfigured LLM provider. Each team member spawns its own agent session that needs a working LLM connection. Check `triggerfish logs` for provider errors around the time of team creation. See [Agent Teams Troubleshooting](/en-GB/support/troubleshooting/security#agent-teams) for more details.
