...
Triggerfish uses two markdown files to define your agent's behaviour:
...
Help Sarah stay organised, informed, and productive. Prioritise calendar
...
::: warning SPINE.md instructions guide the LLM's behaviour but are not security
...
## TRIGGER.md -- Proactive Behaviour
...
Organise your triggers by priority. Be specific about what counts as actionable
and what the agent should do about it.

```markdown
# Priority Checks

- Unread messages across all channels older than 1 hour -- summarise and notify
  on primary channel.
...
- Slack: mentions in #incidents channel -- summarise and escalate if unresolved.
...
- Morning: summarise overnight CI runs and deploy status.
