...
trigger system enables proactive behaviour: scheduled tasks, periodic check-ins,
...
specify monitoring priorities, escalation rules, and proactive behaviours.
...
Use triggers for monitoring that requires judgement (check if anything needs my
...
- If the session taint **exceeds** the trigger's classification, the injection
  ...
  higher-classification session (normal `canFlowTo` behaviour). The session taint
  is unchanged.
