...
The agent can switch models mid-session for cost optimisation. Use a fast model
...
See [Rate Limiting](/en-GB/features/rate-limiting) for full details including OpenAI
tier limits.

::: info API keys are never stored in configuration files. Use your OS keychain
via `triggerfish config set-secret`. See the [Security Model](/en-GB/security/) for
