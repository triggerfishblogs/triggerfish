...
layer is sufficient on its own. Together, they form a defence that degrades
gracefully -- even if one layer is compromised, the remaining layers continue to
protect the system.

::: warning SECURITY Defence in depth means that a vulnerability in any single
...
**Protects against:** Impersonation, unauthorised access, identity confusion.
...
- **Transformations**: How the LLM modified, summarised, or combined the data
...
**Protects against:** Path traversal attacks, unauthorised file access,
...
<img src="/diagrams/data-laundering-defense.svg" alt="Data laundering defence: attack path blocked at ceiling check and taint inheritance prevents output to lower-classification channels" style="max-width: 100%;" />
...
downgrade via memory writes, unauthorised access to classified memories.
