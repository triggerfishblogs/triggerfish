...
<p class="subtitle">Entrez votre adresse e-mail pour recevoir un lien de connexion.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="vous@exemple.com" />
<button id="account-btn">Envoyer le lien</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Consultez votre boîte e-mail pour trouver un lien de connexion. Il vous dirigera vers le portail de facturation où vous pourrez mettre à jour votre forfait, votre moyen de paiement ou annuler.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Une erreur est survenue. Veuillez réessayer.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Impossible de joindre le serveur. Veuillez réessayer.'
    }
