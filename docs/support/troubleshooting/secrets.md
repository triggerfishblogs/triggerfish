...
If the migration involves moving files across filesystem boundaries (different mount points, NFS), the atomic rename may fail. The migration falls back to copy-then-remove, which is still safe but briefly has both files on disk.
...
The resolved value lives only in memory. The config file on disk always contains the `secret:` reference.
