...
| Error | Cause | Fix |
|-------|-------|-----|
| `Secret store failed` | Could not initialize the secret backend | Check keychain/libsecret availability |
...
| `Plugin network access blocked` | Plugin tried to access unauthorized URL | Plugin must declare endpoints in its manifest |
...
| `Workspace agentId rejected: empty after sanitization` | Agent ID contains only invalid characters | Check agent configuration |
