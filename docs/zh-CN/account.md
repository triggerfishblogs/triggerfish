...
<h1>管理您的订阅</h1>
<p class="subtitle">输入您的电子邮件以接收登录链接。</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">发送链接</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = '请检查您的电子邮件以获取登录链接。它将带您到账单门户，您可以在那里更新方案、支付方式或取消订阅。'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || '出现错误，请重试。'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = '无法连接到服务器，请重试。'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = '发送链接'
  })
