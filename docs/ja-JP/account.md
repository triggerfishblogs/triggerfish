...
<h1>サブスクリプションを管理する</h1>
<p class="subtitle">メールアドレスを入力してサインインリンクを受け取ってください。</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">リンクを送信</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'サインインリンクをメールで確認してください。プラン・支払い方法の変更やキャンセルができる請求ポータルに移動します。'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'エラーが発生しました。もう一度お試しください。'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'サーバーに接続できませんでした。もう一度お試しください。'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'リンクを送信'
  })
