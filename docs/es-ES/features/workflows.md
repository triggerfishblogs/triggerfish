...
description: Automatizad tareas de múltiples pasos con el motor CNCF Serverless Workflow DSL integrado en Triggerfish.
---
...
**Usad flujos de trabajo** para secuencias repetibles y deterministas en las que
conocéis los pasos de antemano: obtener datos de una API, transformarlos,
guardarlos en la memoria, enviar una notificación. La misma entrada siempre
produce la misma salida.

**Usad el agente directamente** para razonamiento abierto, exploración o tareas
en las que el siguiente paso depende del criterio: investigar un tema, escribir
código, resolver un problema.

Una buena regla general: si os encontráis pidiendo al agente que haga la misma
secuencia de múltiples pasos repetidamente, convertidla en un flujo de trabajo.
...
`triggerfish:agent` dentro de un flujo de trabajo consume inferencia de vuestro
...
El campo `call` determina el destino del despacho. Consultad
...
herramienta de servidor MCP conectado. Especificad el nombre del `server`, el
...
Este flujo de trabajo obtiene una incidencia de GitHub, la resume con el LLM,
guarda el resumen en la memoria y envía una notificación.
...
Cada tarea puede incluir una directiva `then` que controla lo que sucede
después:
...
**Clasificación de almacenamiento.** Cuando guardáis un flujo de trabajo con
...
clasificación de la sesión en el momento de completarse. `workflow_history`
filtra los resultados por `canFlowTo`, de modo que solo veis ejecuciones que
están en o por debajo del taint de vuestra sesión actual.

::: danger SEGURIDAD
La eliminación de flujos de trabajo requiere que el flujo de trabajo sea
accesible al nivel de clasificación de vuestra sesión actual. No podéis eliminar
un flujo de trabajo almacenado en `CONFIDENTIAL` desde una sesión `PUBLIC`. La
...
autónoma que observa la ejecución en tiempo real, diagnostica fallos y propone
...
clasifica los fallos y coordina equipos de especialistas para resolver los
problemas.

### Habilitar Self-Healing

Añadid un bloque `self_healing` a la sección `metadata.triggerfish` del flujo de
trabajo:
...
2. **Triaje.** Cuando un paso falla, el líder clasifica el fallo en una de cinco
   categorías:
   ...
   | `structural_fix`      | Fallo recurrente que requiere un cambio en la definición del flujo |
...
Hay cuatro herramientas adicionales disponibles para gestionar el estado de
recuperación:
