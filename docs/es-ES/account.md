...
<h1>Gestione su suscripcion</h1>
<p class="subtitle">Introduzca su correo electronico para recibir un enlace de inicio de sesion.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="usted@ejemplo.com" />
<button id="account-btn">Enviar enlace</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Revise su correo electronico para encontrar un enlace de inicio de sesion. Le llevara al portal de facturacion donde puede actualizar su plan, metodo de pago o cancelar.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Ha ocurrido un error. Por favor, intentelo de nuevo.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'No se ha podido contactar con el servidor. Por favor, intentelo de nuevo.'
    }
