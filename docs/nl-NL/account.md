...
<h1>Uw abonnement beheren</h1>
<p class="subtitle">Voer uw e-mailadres in om een aanmeldkoppeling te ontvangen.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="u@voorbeeld.nl" />
<button id="account-btn">Koppeling verzenden</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Controleer uw e-mail voor een aanmeldkoppeling. Die brengt u naar het factureringsportaal waar u uw abonnement, betaalmethode kunt bijwerken of opzeggen.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Er is iets misgegaan. Probeer het opnieuw.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Kan de server niet bereiken. Probeer het opnieuw.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Koppeling verzenden'
  })
