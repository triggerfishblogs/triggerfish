...
<h1>Hantera din prenumeration</h1>
<p class="subtitle">Ange din e-postadress för att få en inloggningslänk.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="du@example.com" />
<button id="account-btn">Skicka länk</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Kontrollera din e-post för en inloggningslänk. Den tar dig till faktureringsportalen där du kan uppdatera din plan, betalningsmetod eller avbryta.'
      } else {
      ...
      msg.className = 'account-message error'
      msg.textContent = 'Kunde inte nå servern. Försök igen.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Skicka länk'
  })
