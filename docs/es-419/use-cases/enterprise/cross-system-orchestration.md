...
Una empresa mediana que ejecuta procure-to-pay en NetSuite, Coupa, DocuSign y Slack define un flujo de trabajo de Triggerfish que maneja el ciclo completo. Los pasos deterministas manejan las llamadas API para crear órdenes de compra, enrutar aprobaciones y conciliar facturas. Los pasos de subagente LLM manejan las excepciones: facturas con ítems que no coinciden con la orden de compra, proveedores que enviaron documentación en un formato inesperado, solicitudes de aprobación que necesitan contexto sobre el historial del solicitante.
...
Cuando Coupa actualiza su API y cambia un nombre de campo, el paso HTTP determinista del flujo de trabajo falla. El motor delega a un subagente que lee el error, identifica el campo cambiado y reintenta con el parámetro correcto. El flujo de trabajo se completa sin intervención humana, y el incidente queda registrado para que un ingeniero actualice la definición del flujo de trabajo para manejar el nuevo formato en adelante.
