...
<p class="subtitle">Ingresa tu correo electrónico para recibir un enlace de inicio de sesión.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="tu@ejemplo.com" />
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Revisa tu correo electrónico para un enlace de inicio de sesión. Te llevará al portal de facturación donde puedes actualizar tu plan, método de pago o cancelar.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Algo salió mal. Por favor intenta de nuevo.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'No se pudo conectar al servidor. Por favor intenta de nuevo.'
    }
