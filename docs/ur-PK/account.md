---
title: Subscription Manage کریں
layout: page
---
...
<h1>اپنی subscription manage کریں</h1>
<p class="subtitle">Sign-in link receive کرنے کے لیے اپنا email درج کریں۔</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">Link بھیجیں</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Sign-in link کے لیے اپنا email چیک کریں۔ یہ آپ کو billing portal تک لے جائے گا جہاں آپ اپنا plan، payment method update کر سکتے ہیں یا cancel کر سکتے ہیں۔'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'کچھ غلط ہو گیا۔ دوبارہ کوشش کریں۔'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Server تک نہیں پہنچ سکے۔ دوبارہ کوشش کریں۔'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Link بھیجیں'
  })
