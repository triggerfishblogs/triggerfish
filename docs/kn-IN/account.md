...
<h1>ನಿಮ್ಮ subscription ನಿರ್ವಹಿಸಿ</h1>
<p class="subtitle">Sign-in link ಪಡೆಯಲು ನಿಮ್ಮ email ನಮೂದಿಸಿ.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">Link ಕಳುಹಿಸಿ</button>
</div>
...
onMounted(() => {
  ...
  btn.addEventListener('click', async () => {
    ...
    btn.disabled = true
    btn.textContent = 'ಕಳುಹಿಸುತ್ತಿದ್ದೇವೆ...'
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Sign-in link ಗಾಗಿ ನಿಮ್ಮ email ತಪಾಸಣೆ ಮಾಡಿ. ಅದು ನಿಮ್ಮನ್ನು billing portal ಗೆ ಕರೆದೊಯ್ಯುತ್ತದೆ, ಅಲ್ಲಿ ನೀವು ನಿಮ್ಮ plan, payment method update ಮಾಡಬಹುದು ಅಥವಾ cancel ಮಾಡಬಹುದು.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'ಏನೋ ತಪ್ಪಾಯಿತು. ದಯವಿಟ್ಟು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Server ತಲುಪಲಾಗಲಿಲ್ಲ. ದಯವಿಟ್ಟು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Link ಕಳುಹಿಸಿ'
  })
