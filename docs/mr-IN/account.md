...
<h1>तुमची subscription manage करा</h1>
<p class="subtitle">Sign-in link मिळवण्यासाठी तुमचा email टाका.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">Link पाठवा</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Sign-in link साठी तुमचा email तपासा. ते तुम्हाला billing portal मध्ये घेऊन जाईल जेथे तुम्ही तुमची plan, payment method update करू शकता किंवा cancel करू शकता.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'काहीतरी चुकले. कृपया पुन्हा प्रयत्न करा.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Server पर्यंत पोहोचता आले नाही. कृपया पुन्हा प्रयत्न करा.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Link पाठवा'
  })
