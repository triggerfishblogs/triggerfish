...
[cron and trigger system](/features/cron-and-triggers). A webhook event can:
...
- **Update trigger priorities** (for example, a Sentry alert makes the agent
  prioritize error investigation on its next trigger wakeup)
...
See the full [GitHub Integration](/integrations/github) guide for setup
...
port forwarding, a reverse proxy, or a tunnel. The [Remote Access](/reference/)
