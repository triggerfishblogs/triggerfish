...
<h1>ניהול המנוי שלכם</h1>
<p class="subtitle">הזינו את כתובת הדוא"ל שלכם כדי לקבל קישור כניסה.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">שליחת קישור</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'בדקו את הדוא"ל שלכם לקבלת קישור כניסה. הקישור יוביל אתכם לפורטל החיוב שבו תוכלו לעדכן את התוכנית, אמצעי התשלום או לבטל.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'משהו השתבש. אנא נסו שוב.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'לא ניתן להתחבר לשרת. אנא נסו שוב.'
    }
