...
<h1>Administrer abonnementet ditt</h1>
<p class="subtitle">Skriv inn e-postadressen din for å motta en innloggingslenke.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="deg@eksempel.com" />
<button id="account-btn">Send lenke</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = 'Sjekk e-posten din for en innloggingslenke. Den tar deg til fakturaportalen der du kan oppdatere planen din, betalingsmetoden, eller si opp abonnementet.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || 'Noe gikk galt. Prøv igjen.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = 'Kunne ikke nå serveren. Prøv igjen.'
    }

    msg.style.display = ''
    btn.disabled = false
    btn.textContent = 'Send lenke'
  })
