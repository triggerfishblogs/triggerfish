...
<h1>구독 관리</h1>
<p class="subtitle">로그인 링크를 받으려면 이메일을 입력하십시오.</p>
<div class="account-form">
<input type="email" id="account-email" placeholder="you@example.com" />
<button id="account-btn">링크 전송</button>
</div>
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        msg.textContent = '이메일에서 로그인 링크를 확인하십시오. 요금제 업데이트, 결제 방법 변경 또는 취소가 가능한 청구 포털로 이동합니다.'
      } else {
        const data = await resp.json().catch(() => ({}))
        msg.className = 'account-message error'
        msg.textContent = data.error || '문제가 발생했습니다. 다시 시도하십시오.'
      }
    } catch {
      msg.className = 'account-message error'
      msg.textContent = '서버에 연결할 수 없습니다. 다시 시도하십시오.'
    }
