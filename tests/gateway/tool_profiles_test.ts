...
Deno.test("triggerSession profile excludes browser tools", () => {
  ...
  for (const name of browserNames) {
    assertEquals(names.has(name), false, `triggerSession should not have ${name}`);
  }
});
...
Deno.test("triggerSession profile includes exec tools", () => {
  const names = toolNamesFor("triggerSession");
...
Deno.test("tidepool profile is a superset of cli profile", () => {
  ...
  for (const name of cliNames) {
    assertEquals(tidepoolNames.has(name), true, `tidepool should have cli tool ${name}`);
  }
});
...
Deno.test("cli profile is a superset of triggerSession profile", () => {
  ...
  for (const name of triggerNames) {
    assertEquals(cliNames.has(name), true, `cli should have trigger tool ${name}`);
  }
});
...
Deno.test("triggerSession prompts exclude tidepool/plan/session/image/explore/claude/secrets prompts", () => {
  ...
  for (const prompt of excluded) {
    if (prompt) {
      assertEquals(prompts.includes(prompt), false,
...
Deno.test("triggerSession prompts include web/todo/memory prompts", () => {
  ...
  for (const prompt of included) {
    if (prompt) {
      assertEquals(prompts.includes(prompt), true,
...
Deno.test("resolveToolsForProfile accepts ad-hoc group list", () => {
  ...
  const execNames = TOOL_GROUPS.exec().map((t) => t.name);
  for (const name of execNames) {
    assertEquals(names.has(name), true, `ad-hoc should have exec tool ${name}`);
  }
  ...
  const browserNames = TOOL_GROUPS.browser().map((t) => t.name);
  for (const name of browserNames) {
    assertEquals(names.has(name), false, `ad-hoc should not have browser tool ${name}`);
  }
});
