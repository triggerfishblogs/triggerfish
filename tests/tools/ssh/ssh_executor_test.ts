...
Deno.test("ssh_execute rejects missing host", async () => {
  ...
  assertStringIncludes(result!, "Error: ssh_execute requires a non-empty 'host'");
});
...
Deno.test("ssh_execute rejects missing command", async () => {
  ...
  assertStringIncludes(result!, "Error: ssh_execute requires a non-empty 'command'");
});

Deno.test("ssh_execute rejects empty command", async () => {
  const executor = createSshToolExecutor();
  const result = await executor("ssh_execute", { host: "example.com", command: "" });
  assertStringIncludes(result!, "Error: ssh_execute requires a non-empty 'command'");
});
...
Deno.test("ssh_session_open rejects missing host", async () => {
  ...
  assertStringIncludes(result!, "Error: ssh_session_open requires a non-empty 'host'");
});

Deno.test("ssh_session_write rejects missing session_id", async () => {
  const executor = createSshToolExecutor();
  const result = await executor("ssh_session_write", { input: "hello" });
  assertStringIncludes(result!, "Error: ssh_session_write requires a non-empty 'session_id'");
});
...
Deno.test("ssh_session_read rejects missing session_id", async () => {
  ...
  assertStringIncludes(result!, "Error: ssh_session_read requires a non-empty 'session_id'");
});

Deno.test("ssh_session_close rejects missing session_id", async () => {
  const executor = createSshToolExecutor();
  const result = await executor("ssh_session_close", {});
  assertStringIncludes(result!, "Error: ssh_session_close requires a non-empty 'session_id'");
});
