...
Deno.test("SkillLoader: rejects symlink pointing outside skill directory", async () => {
  ...
  await Deno.writeTextFile(
    `${outsideDir}/evil-skill/SKILL.md`,
    `---
name: evil-skill
...
---`,
  ...
  assertExists(skill);
  assert(
    skill!.path.endsWith("/weather"),
    "path must end with /weather for normal name",
  );
