 ...
 */
export function resolveToolsForProfile(profile: ToolProfileName | ToolProfile): readonly ToolDefinition[] {
 ...
 */
export function resolvePromptsForProfile(profile: ToolProfileName | ToolProfile): readonly string[] {
  const groups: ToolProfile = typeof profile === "string"
    ? TOOL_PROFILES[profile] ?? []
    : profile;
  return groups
