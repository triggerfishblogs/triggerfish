/**
 * Expression evaluation engine for workflow contexts.
 *
 * Handles `${ .path.to.value }` interpolation syntax, dot-path resolution,
 * comparison and arithmetic operators, literal parsing, and deep object
 * resolution.
 * @module
 */
...
/** Resolve a dot-path like `.result.items[0].name` against data. */
...
/** Resolve an expression value -- either a literal or a dot-path. */
function resolveExprValue(
  ...
  // Dot-path
  return resolveDotPath(data, expr);
}
...
/** Evaluate a `${ expression }` string, returning raw value for single expressions. */
...
export function deepResolveExpressions(
  data: Readonly<Record<string, unknown>>,
  value: unknown,
): unknown {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => deepResolveExpressions(data, item));
  }

  if (value !== null && typeof value === "object") {
    const resolved: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(value)) {
      resolved[k] = deepResolveExpressions(data, v);
    }
    return resolved;
  }
...
/** Check whether a string contains `${ }` expression syntax. */
export function containsExpression(value: string): boolean {
  const result = EXPR_PATTERN.test(value);
  EXPR_PATTERN.lastIndex = 0;
  return result;
}
