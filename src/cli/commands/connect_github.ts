...
/** Fetch the GitHub user endpoint with a PAT using SSRF-checked HTTP. */
function fetchGithubUser(
  token: string,
): ReturnType<typeof safeFetch> {
  return safeFetch("https://api.github.com/user", {
    headers: {
...
async function verifyGithubToken(token: string): Promise<string | null> {
  ...
  if (!result.ok) {
    log.error("GitHub API request failed", { operation: "connectGithub", err: result.error });
