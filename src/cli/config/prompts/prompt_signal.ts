...
async function fetchAndInstallSignalCli(): Promise<SignalCliBinary> {
  ...
  return { path: installResult.value.path, javaHome: installResult.value.javaHome };
}

/** Initiate the link process and display the QR code. */
async function initiateDeviceLink(
  binary: SignalCliBinary,
): Promise<{ uri: string; process: { status: Promise<{ success: boolean }> } }> {
...
async function ensureSignalDaemon(
  ...
  const alreadyRunning = await isDaemonRunning(SIGNAL_TCP_HOST, SIGNAL_TCP_PORT);
  if (alreadyRunning) {
    console.log(`  signal-cli daemon already running on ${SIGNAL_TCP_HOST}:${SIGNAL_TCP_PORT}`);
    return;
  }

  const startIt = await Confirm.prompt({
    message: `Start signal-cli daemon on tcp://${SIGNAL_TCP_HOST}:${SIGNAL_TCP_PORT}?`,
    default: true,
  });
...
function printManualDaemonInstructions(account: string, binaryPath: string): void {
  console.log("\n  Start it manually before running Triggerfish:");
  console.log(`  ${binaryPath} -a ${account} daemon --tcp ${SIGNAL_TCP_HOST}:${SIGNAL_TCP_PORT}\n`);
}
...
async function spawnSignalDaemon(
  ...
  const daemonResult = startDaemon(
    account, SIGNAL_TCP_HOST, SIGNAL_TCP_PORT,
    binary.path, binary.javaHome,
  );
...
async function reportDaemonStartupFailure(
  ...
  console.error(`  Check: ${binaryPath} -a ${account} daemon --tcp localhost:7583`);
}
...
async function promptSignalGroupAndClassification(): Promise<Record<string, unknown>> {
