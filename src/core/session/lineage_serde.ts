...
/** Storage key for the forward trace reverse index. */
export function lineageFwdIndexKey(
  parentId: string,
  childId: string,
): string {
...
/** Storage key prefix for listing forward trace children. */
export function lineageFwdPrefixKey(parentId: string): string {
  return `lineage-fwd:${parentId}:`;
}

/** Storage key for the hash-to-lineage index. */
